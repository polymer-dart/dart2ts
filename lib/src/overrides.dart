...
abstract class Translator {
  ...
  String getProperty(DartType targetType, PropertyAccessorElement getter, String target, String propertyName);
  String setProperty(
      DartType targetType, PropertyAccessorElement setter, String target, String propertyName, String value);

  String getField(FieldElement field, String target, String propertyName);
  String setField(FieldElement field, String target, String propertyName, String value);

  String indexGet(DartType targetType, String target, String index);

  bool checkIndexed(DartType targetType);

  String indexSet(DartType targetType, String target, String index, String value);
}
...
class DefaultTranslator implements Translator {
  ...
  static _target(String target) => target != null && target.isNotEmpty ? "${target}." : "";
  ...
  @override
  String binaryOp(MethodElement operator, String op, String left, String right) => "${left} ${op} ${right}";

  @override
  String getField(FieldElement field, String target, String propertyName) => "${target}.${propertyName}";
  ...
  @override
  String indexGet(DartType targetType, String target, String index) => "${target}[${index}]";
  @override
  String indexSet(DartType targetType, String target, String index, String value) {
...
class TranslatorBase implements Translator {
  ...
  @override
  String binaryOp(MethodElement operator, String op, String left, String right) => throw "Not Implemented";
  ...
  @override
  String getField(FieldElement field, String target, String propertyName) => throw "Not Implemented";
  ...
  @override
  String setField(FieldElement field, String target, String propertyName, String value) => throw "Not Implemented";
  ...
  @override
  String indexGet(DartType targetType, String target, String index) => throw "Not Implemented";
  @override
  String indexSet(DartType targetType, String target, String index, String value) => throw "Not Implemented";
}
...
class ListTranslator extends TranslatorBase {
  ...
  bool checkAccessor(DartType targetType, PropertyAccessorElement accessor) {
    ...
    return enclosing is ClassElement && isListType(targetType) && _overriddenAccessors.contains(accessor.name);
  }

  static final Set<String> _overriddenAccessors = new Set.from(const ['first', 'last', 'isEmpty', 'isNotEmpty']);
  ...
  @override
  bool checkMethod(MethodElement method, Expression targetExpression) {
    return method != null && (isListType(method.enclosingElement.type)) && _overriddenMethodNames.contains(method.name);
  }
...
class IterableTranslator extends TranslatorBase {
  ...
  bool checkAccessor(DartType targetType, PropertyAccessorElement accessor) {
    ...
    Element enclosing = accessor.enclosingElement;
    return enclosing is ClassElement && isIterableType(targetType) && _overriddenAccessors.contains(accessor.name);
  }

  static final Set<String> _overriddenAccessors = new Set.from(const ['first', 'last', 'isEmpty', 'isNotEmpty']);

  static final Set<String> _overriddenMethodNames = new Set.from(
      const ['add', 'remove', 'from', 'take', 'removeLast', 'map', 'where', 'firstWhere', 'join', 'lastWhere']);
...
class NumberTranslator extends TranslatorBase {
  ...
  static final Set<String> _overriddenMethodNames = new Set.from(const ['parse']);

  @override
  bool checkMethod(MethodElement method, Expression targetExpression) {
    return (targetExpression?.staticType == currentContext.typeProvider.numType) &&
        _overriddenMethodNames.contains(method.name);
  }
...
class IntTranslator extends TranslatorBase {
  ...
  @override
  bool checkMethod(MethodElement method, Expression targetExpression) {
    return (targetExpression?.staticType == currentContext.typeProvider.intType) &&
        _overriddenMethodNames.contains(method.name);
  }
...
class StringTranslator extends TranslatorBase {
  ...
  static final Set<String> _overriddenAccessors = new Set.from(const ['codeUnits', 'isEmpty', 'isNotEmpty']);

  static final Set<String> _overriddenMethodNames =
      new Set.from(const ['codeUnitAt', 'replaceAll', 'take', 'contains', 'allMatches']);

  @override
  bool checkMethod(MethodElement method, Expression targetExpression) {
    return method != null &&
        (targetExpression?.staticType == method.context.typeProvider.stringType) &&
        _overriddenMethodNames.contains(method.name);
  }
...
class TranslatorRegistry {
  ...
  String newInstance(ConstructorElement constructor, String className, List<String> arguments) =>
      _find((t) => t.checkNewInstance(constructor)).newInstance(constructor, className, arguments);

  String invokeMethod(MethodElement method, Expression targetExpression, String target, String methodName,
          List<String> arguments) =>
      _find((t) => t.checkMethod(method, targetExpression)).invokeMethod(method, target, methodName, arguments);
  String getProperty(DartType targetType, PropertyAccessorElement getter, String target, String propertyName) =>
      _find((t) => t.checkAccessor(targetType, getter)).getProperty(targetType, getter, target, propertyName);

  String setProperty(
          DartType targetType, PropertyAccessorElement setter, String target, String propertyName, String value) =>
      _find((t) => t.checkAccessor(targetType, setter)).setProperty(targetType, setter, target, propertyName, value);

  String getField(FieldElement field, String target, String propertyName) =>
      _find((t) => t.checkField(field)).getField(field, target, propertyName);

  String setField(FieldElement field, String target, String propertyName, String value) =>
      _find((t) => t.checkField(field)).setField(field, target, propertyName, value);

  String indexGet(DartType targetType, String target, String index) =>
      _find((t) => t.checkIndexed(targetType)).indexGet(targetType, target, index);

  String indexSet(DartType targetType, String target, String index, String value) =>
      _find((t) => t.checkIndexed(targetType)).indexSet(targetType, target, index, value);
}
